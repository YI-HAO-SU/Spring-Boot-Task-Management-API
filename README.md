...
**目的**：整個應用全程只用一個統計 cache 實例，確保所有請求共享同一份快取狀態。

**實現方式**：
```java
// AppConfig.java
@Bean
@Scope("singleton")  // Spring 預設就是 singleton，明確宣告表達設計意圖
public TaskStatisticsCache taskStatisticsCache() {
    return new TaskStatisticsCache();
}

// TaskStatisticsCache.java
private final Map<String, TaskStatsResponse> cache = new ConcurrentHashMap<>();

public TaskStatsResponse compute(List<Task> tasks) {
    return cache.computeIfAbsent("stats", k -> calculate(tasks));
}

public void invalidate() {
    cache.clear();  // 任何寫入操作後呼叫，確保資料一致性
}
```

**Cache 生命週期**：
```
GET /api/tasks/stats (第一次) → cache miss → calculate() → 存入 Map
GET /api/tasks/stats (第二次) → cache hit  → 直接回傳，不重算
POST / PUT / PATCH / DELETE   → invalidate() → 清空 cache
GET /api/tasks/stats (寫入後) → cache miss → 重新計算
```

**為什麼這樣設計**：Singleton 確保全應用只有一個 `ConcurrentHashMap` 實例，不會有多個 cache 各自存不同版本的統計結果。用 Spring `@Bean` 管理比手寫 `getInstance()` 更符合依賴注入原則，`TaskStatisticsCache` 本身保持 POJO，方便單元測試直接 `new`。
...
```bash
# 第一次呼叫 → cache miss，重新計算
curl http://localhost:8080/api/tasks/stats

# 第二次呼叫 → cache hit，不重算（終端機不會再印 log）
curl http://localhost:8080/api/tasks/stats

# 更新一筆任務後 → cache invalidated
curl -X PATCH http://localhost:8080/api/tasks/1/status \
  -H "Content-Type: application/json" \
  -d '{"status": "DONE"}'

# 再次查詢 → cache miss，重新計算
curl http://localhost:8080/api/tasks/stats
```

終端機輸出：
```
[SINGLETON] TaskStatisticsCache instance #123456789 computing stats   ← 第一次
[SINGLETON] TaskStatisticsCache instance #123456789 cache invalidated ← 寫入後
[SINGLETON] TaskStatisticsCache instance #123456789 computing stats   ← 重新計算
```

instance 號碼永遠相同 ← 這就是 Singleton；第二次查詢沒有 log ← 這就是 Cache hit。
